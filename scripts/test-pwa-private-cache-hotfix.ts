 ...
 *
 * Self-contained on purpose: it imports no application code, so the security
 * policy it guards is checked the same way on main and on the release branch.
...
async function main() {
  ...
  /*
    The hotfix shipped its retry button beside the page as app/offline/RetryButton.tsx.
    The release branch keeps it with the other PWA components, and that copy is
    the one that ships next, so the checks follow it there.
  */
  const RETRY_COMPONENT = 'components/pwa/OfflineRetryButton.tsx'
  const retry = fs.existsSync(path.join(ROOT, RETRY_COMPONENT)) ? code(RETRY_COMPONENT) : ''
  check(
  ...
  check('O22 retry is a real button the person presses', retry.includes("'use client'") && /onClick=\{retry\}/.test(retry) && offline.includes('<OfflineRetryButton />'), true)
